...
begin
  ...
  BEGIN   
     ...
     BEGIN
      select @errno  = 30002,
             @errmsg = 'Impossível Incluir #VENDAS_LOTE# porque o #VALOR DA PARCELA#('+RTRIM(CAST(CAST((@tot_valor_liquido / @numeroParcelas) AS numeric(14,2)) AS CHAR(20)))+') é menor que valor mínimo permitido ('+RTRIM(CAST(@valor_minimo_parcela as char(20)))+').'
      goto error
     END     

     -- Verifica TOT_QTDE_ORIGINAL x CONDIÇÃO DE PAGAMENTO
     if (@condicaoPgto in ('AL','BZ','BN') AND @tot_valor_liquido < 5000.00)
     BEGIN
      select @errno  = 30002,
             @errmsg = 'Impossível Incluir #VENDAS_LOTE# porque #TOT_VALOR_LIQUIDO# do pedido é menor que mínimo permitido (R$ 5.000,00) para esta #CONDIÇÃO DE PAGAMENTO#'
      goto error
     END     
