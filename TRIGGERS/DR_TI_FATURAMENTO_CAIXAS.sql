create trigger [dbo].[DR_TI_FATURAMENTO_CAIXAS] 
  ...
  begin
	...
	if (@filial = @filial_loja_func) 
		begin
			...
			begin
			  select @errno  = 30007,
					 @errmsg = 'Impossível Incluir #VENDAS_PROD_EMBALADO# Porque quantidade de peças está maior que permitido ('+ rtrim(cast(@max_qtde_pecas_venda_func as char(10))) +') pçs'
			  goto error
			end
		end
  end
