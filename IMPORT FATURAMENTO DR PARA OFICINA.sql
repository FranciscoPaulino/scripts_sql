EXEC IMPORTA_FATURAMENTO_DR_PARA_OFICINA 'LANÇADORA'
...
WHERE NOT EXISTS(SELECT * FROM FATURAMENTO WHERE NF_SAIDA=A.NF_SAIDA AND SERIE_NF=A.SERIE_NF AND NOME_CLIFOR=A.NOME_CLIFOR) AND A.FILIAL=@P_FILIAL
...
BEGIN  
	...
	AND A.FILIAL=@P_FILIAL and A.nf_saida=@NF_SAIDA AND A.SERIE_NF=@SERIE_NF
	...
	AND A.FILIAL=@P_FILIAL AND A.nf_saida=@NF_SAIDA AND A.SERIE_NF=@SERIE_NF AND C.INATIVO=0

	--- GERAR IMPOSTOS DE SAIDA
	EXEC LX_GERA_IMPOSTOS_SAIDA  @P_FILIAL,@NF_SAIDA,@SERIE_NF
