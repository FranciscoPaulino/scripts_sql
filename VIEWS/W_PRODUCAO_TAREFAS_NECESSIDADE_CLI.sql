...
/****** Object:  View [dbo].[W_PRODUCAO_TAREFAS_NECESSIDADE]    Script Date: 12/02/2015 11:01:36 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[W_PRODUCAO_TAREFAS_NECESSIDADE_CLI]
AS  
...
FROM  W_NECESSIDADE_ESTOQUE_VENDAS_CLI A  
  LEFT JOIN   
    (SELECT A.PRODUTO,A.COR_PRODUTO,B.SEQUENCIA_PRODUTIVA,B.FASE_PRODUCAO, ISNULL(SUM(QTDE_S),0) AS QTDE_FASE,  
     ...
     WHERE D.FILIAL LIKE RTRIM(LTRIM(ISNULL((SELECT VALOR_ATUAL FROM PARAMETROS WHERE PARAMETRO ='VENDA_ESTOQUE_FILIAL_CLI' ),'%'))) AND 
                                         B.SETOR_PARALELO_NUMERO <= 1   
