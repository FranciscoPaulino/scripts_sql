...
GO
GO
if object_id('stpCarga_Tamanhos_Tabelas') is not null
	drop procedure stpCarga_Tamanhos_Tabelas
GO

create proc [dbo].stpCarga_Tamanhos_Tabelas
as
	...
	FROM ##Tamanho_Tabelas A
		LEFT JOIN dbo.Servidor B ON A.Nm_Servidor = B.Nm_Servidor collate Latin1_General_CI_AS
	WHERE B.Nm_Servidor IS null
		
	INSERT INTO dbo.BaseDados(Nm_Database)
	SELECT DISTINCT A.Nm_Database 
	FROM ##Tamanho_Tabelas A
		LEFT JOIN dbo.BaseDados B ON A.Nm_Database = B.Nm_Database collate Latin1_General_CI_AS
	WHERE B.Nm_Database IS null
	
	INSERT INTO dbo.Tabela(Nm_Tabela)
	SELECT DISTINCT A.Nm_Tabela 
	FROM ##Tamanho_Tabelas A
		LEFT JOIN dbo.Tabela B ON A.Nm_Tabela = B.Nm_Tabela collate Latin1_General_CI_AS
	...
	from ##Tamanho_Tabelas A
		JOIN dbo.Servidor B ON A.Nm_Servidor = B.Nm_Servidor collate Latin1_General_CI_AS
		JOIN dbo.Tabela C ON A.Nm_Tabela = C.Nm_Tabela collate Latin1_General_CI_AS
		JOIN dbo.BaseDados D ON A.Nm_Database = D.Nm_Database collate Latin1_General_CI_AS
...
exec stpCarga_Tamanhos_Tabelas

--- consulta maiores tabelas
SELECT top 10 *
